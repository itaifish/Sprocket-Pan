...
import { WorkspaceDataManager, workspaceDataManager } from './WorkspaceDataManager';
...
class FileSystemManager extends EventEmitter<FileSystemEvent> {
	...
	async getDirectories(rootFolder: string): Promise<string[]> {
		try {
			const directoryNames = await readDir(rootFolder, { dir: WorkspaceDataManager.DEFAULT_DIRECTORY });
	...
	async getWorkspaces() {
		...
		const workspaceFolders = [undefined, ...(await this.getDirectories(WorkspaceDataManager.DATA_FOLDER_NAME))];
	...
	async deleteWorkspace(path: string) {
		const paths = workspaceDataManager.getWorkspacePath(path);
		const doesExist = await exists(paths.metadata, {
			dir: WorkspaceDataManager.DEFAULT_DIRECTORY,
		});
		if (!doesExist) {
