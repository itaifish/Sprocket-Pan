 ...
 *
 * Depth 0 is the classic {...spread, ...merge}. Depth 1 performs spread merges on any direct properties that have the same name and are both objects. Depth 1 does that to the descendents of descendents, etc. Default depth is 5.
 *
 * !! Warning: high depths and large objects may cause silent failures.
 */
export function mergeDeep<T, J extends RecursivePartial<T>>(obj1: T, obj2: J, depth = 5): T & J {
	if (!isRecord(obj1) || !isRecord(obj2)) {
		return (obj2 !== undefined ? structuredClone(obj2) : structuredClone(obj1)) as T & J;
	}
	...
	for (const key in obj2Copy) {
		...
		// @ts-ignore
		obj1Copy[key] = mergeDeep(obj1Copy[key], obj2Copy[key], depth--);
	}
